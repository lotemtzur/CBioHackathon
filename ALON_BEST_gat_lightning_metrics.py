...
import utils
from gnn_lightning import GCNEncoder
...
class GATEncoder(nn.Module):
    def __init__(self, in_dim, hidden_dim, z_dim):
        super().__init__()
        # heads=4 allows the model to learn 4 different types of relationships
        self.conv1 = GATConv(in_dim, hidden_dim, heads=4, concat=True)

        # We project the concatenated heads down to z_dim
        # Input dim is hidden_dim * heads
        self.mu = GATConv(hidden_dim * 4, z_dim, heads=1, concat=False)
        self.logvar = GATConv(hidden_dim * 4, z_dim, heads=1, concat=False)
...
def visualize_latent_space(z_train, z_test_predicted, method="tsne"):
    """
    Plots the structural embeddings of the Train set vs. the
    Predicted structural embeddings of the Cold-Start Test set.
    """
    print(f"\nComputing {method.upper()} visualization...")

    # Move to CPU numpy
    z_train_np = z_train.detach().cpu().numpy()
    z_test_np = z_test_predicted.detach().cpu().numpy()

    # Concatenate for dimensionality reduction
    X = np.concatenate([z_train_np, z_test_np], axis=0)

    # Reduce dims (TSNE is usually better for this than PCA)
    reducer = TSNE(n_components=2, random_state=42, perplexity=30, n_iter=1000)
    X_embedded = reducer.fit_transform(X)

    # Split back
    train_emb = X_embedded[:len(z_train_np)]
    test_emb = X_embedded[len(z_train_np):]

    # Plot
    plt.figure(figsize=(10, 8))

    # Plot Train nodes (Light Blue, smaller)
    plt.scatter(train_emb[:, 0], train_emb[:, 1],
                alpha=0.3, s=30, c='dodgerblue', label='Train (Known Structure)')

    # Plot Test nodes (Red, larger)
    plt.scatter(test_emb[:, 0], test_emb[:, 1],
                alpha=0.7, s=50, c='crimson', marker='^', label='Test (Predicted from Sequence)')

    plt.title(f"Cold Start: Sequence-to-Structure Mapping ({method.upper()})")
    plt.legend()
...
def main():
    ...
    # We need the PREDICTED Z for the test set, so let's run the MLP one last time
    mlp_model.eval()
    with torch.no_grad():
        z_test_pred = mlp_model(esm_test)

    visualize_latent_space(z_train_structure, z_test_pred)
